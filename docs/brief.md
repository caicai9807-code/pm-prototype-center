...
| 站点卡片区 | 名称、编码、检查状态、整改状态、地址、办事处、片区所 | 点击卡片 | 检查状态与整改状态独立展示；无需整改统一显示“—” |
...
| 页面区域 | 展示内容与关键字段 | 操作入口 | 动态规则与区域关系 |
| --- | --- | --- | --- |
| 状态卡 | 站点、结论、完成时间、确认进度、检查状态、整改状态 | 检查确认、整改审核 | 检查操作由检查状态控制，整改操作由整改状态控制；已结束任务仅展示 |
...
**交互逻辑**：检查状态限定为待检查、检查中、已检查；整改状态限定为—、待整改、整改中、已整改、超期。两类状态分别驱动检查操作与整改操作，互不覆盖。Tab 切换按需加载并保留已加载状态；上传照片失败可单张重试。复核成功后以服务端结果刷新建筑卡片，不能依赖页面内存。审核提交后更新状态卡和确认进度，并同步 Web 端。
