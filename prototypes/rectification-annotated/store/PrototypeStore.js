...
(function () {
  ...
  // 标注版使用独立缓存，避免与原版原型互相覆盖演示数据。
  var STORAGE_KEY = 'prototype_rectification_store_annotated';
  var STORAGE_VERSION_KEY = 'prototype_rectification_store_annotated_version';
  var STORAGE_VERSION = 'expert-mixed-sites-v1-20260714';
  ...
  function buildSeed() {
    ...
    // 标注版重置后仅保留一条合格记录和一条待整改记录。
    // 其余站点不携带已提交检查、会签、整改或复查历史，避免与演示状态冲突。
    delete checkRecords["60218B011"];
    ...
    function genNoticeFromCR(siteCode, cr) {
      ...
      if (siteCode === '60218B005') deadlineDays = 15; // ≤30天=审核通过，进入待整改
      return {
        id: noticeIdSeq,
    ...
    // 专家 APP 演示任务保留四种典型站点状态。
    if (siteStates["60218B004"]) siteStates["60218B004"].status = '待检查';
    if (siteStates["60218B003"]) siteStates["60218B003"].status = '检查中';
    if (siteStates["60218B005"]) siteStates["60218B005"].status = '待整改';
    if (siteStates["60218B007"]) siteStates["60218B007"].status = '已检查';
